...
log_file_exceptions = open("/Users/shreejakatama/Downloads/Internship/Folder Code/Log File Exceptions.txt", 'a')
log_file_success = open("/Users/shreejakatama/Downloads/Internship/Folder Code/Log File Success.txt", 'a')
...
def check_rtf(file_path):
    ...
    for i in rtf_tags:
        if i in rtf_content:
            ...
            debug_print(i + " not in rtf") # If RTF tag is not present, the RTF does not adhere to the schema
...
def extract_header(page_content):
    ...
    try:
        ...
        headerstyle = re.findall(RE_expressions['headerstyle'] , page_content[header_start : header_end])
        
        # Dictionary to store the header content along with the alignment
        headers = {}
        for h in range (len(header)):
            header[h] = header[h].replace('{\\field{\\*\\fldinst { PAGE }}}{',str(PAGE)).replace('}{\\field{\\*\\fldinst { NUMPAGES }}}',str(NUMPAGES))
            headers[header[h]] = Header_alignment[headerstyle[h][0]]
...
def extract_footnotes(page_content):
    ...
    # Used to check whether the footnotes are extracted successfully
        ...
        log_file_exceptions.write("Footnotes not extracted successfully in page " + str(PAGE) + "\n")
        footnotes = []
    return footnotes
